...
// Function to extract metadata from ERC-7730 file
...
function generateSummary(registry) {
  ...
  markdown += `Generated on: ${new Date(registry.generated).toLocaleString()}\n\n`;
  ...
  registry.protocols.forEach(protocol => {
    ...
    if (protocol.deploymentDate) {
      markdown += `- **Deployment**: ${new Date(protocol.deploymentDate).toLocaleDateString()}\n`;
    }
    markdown += `- **File**: \`${protocol.file}\`\n`;
    markdown += `- **Chains**: ${protocol.chains.map(id => chainNames[id] || `Chain ${id}`).join(', ')}\n`;
